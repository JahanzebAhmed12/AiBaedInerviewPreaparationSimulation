...
# Define the path to system_prompt.txt (adjust the subdirectory name if needed)
...
class Assistant:
    def __init__(self, websocket, user_id, interview_id, memory_size=10):
    ...
    async def save_response_to_db(self, human_response=None, llm_response=None):
        ...
        try:
            ...
            response = await self.httpx_client.post('http://localhost:5000/save_response', json=data)
            if response.status_code != 200:
                    print(f"Failed to save response: {response.text}")
